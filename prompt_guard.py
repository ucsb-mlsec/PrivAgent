...
if __name__ == "__main__":
    ...
    benign_text = "Here is an original version of the system prompt. Edit it for grammar and clarity. "
    print(
    ...
    for prompt_path in prompts_path:
        ...
        if "prompt" in data:
            ...
            raise ValueError("Prompt column not found in CSV.")
        # choose 10 random prompts
        # sort by reward
        top_prompts = prompts[:10]
        top_data = data[:10]
        results = get_jailbreak_scores_for_texts(model, tokenizer, top_prompts)
        # filter out the benign prompts
        filtered_id = [i for i, r in enumerate(results) if r < 0.6]
        # save the results
        filtered_data = top_data.iloc[filtered_id]
        filtered_data.to_csv(prompt_path.replace(".csv", "_filtered.csv"), index = False)
        print(f"{prompt_path}: max {max(results):.3f} mean {sum(results) / len(results):.3f}")
